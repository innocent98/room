...
import { Layout, Typography, Form, Button, message, Result, Spin, Progress } from "antd"
...
export default function FormView() {
  ...
  const handleValuesChange = (changedValues: any, allValues: any) => {
    ...
    formData.fields.forEach((field: any) => {
      ...
      if (field.conditionalLogic.conditions && field.conditionalLogic.conditions.length > 0) {
        field.conditionalLogic.conditions.forEach((condition: any) => {
          ...
          switch (operator) {
            ...
            case "notEquals":
              ...
              break
            case "contains":
              if (Array.isArray(sourceValue) && sourceValue.includes(value)) shouldShow = true
              break
            case "notContains":
              if (Array.isArray(sourceValue) && !sourceValue.includes(value)) shouldShow = true
              break
  ...
  const getFieldsForCurrentStep = () => {
    ...
    const visibleFieldsList = formData.fields.filter((field: any) => visibleFields[field.id])
    const totalFields = visibleFieldsList.length
    const fieldsPerStep = Math.ceil(totalFields / 3) // Divide into 3 steps

    return visibleFieldsList.slice(currentStep * fieldsPerStep, (currentStep + 1) * fieldsPerStep)
  }
  ...
  const handleSignatureChange = (fieldId: string, dataURL: string | null) => {
    if (dataURL) {
      signatureDataRef.current[fieldId] = dataURL
    } else {
      delete signatureDataRef.current[fieldId]
    }
  }
        ...
        <Content className="p-6">
          <div className="max-w-3xl mx-auto">
            <Result
              ...
              subTitle={
                formData.settings?.successMessage || "Thank you for your submission. Your response has been recorded."
              }
              extra={[
  ...
  return (
    <Layout className="min-h-screen">
      ...
      <Content className="p-6">
        <div className="max-w-3xl mx-auto">
          <div className="bg-white p-6 rounded-lg shadow-sm mb-6">
            ...
            {formData.createdBy && <Paragraph className="text-gray-500">Created by: {formData.createdBy}</Paragraph>}
          </div>
                            ...
                            rules={[
                              {
                                ...
                                validator: (_, value) => {
                                  if (field.required && !signatureDataRef.current[field.id]) {
                                    return Promise.reject(`Please sign ${field.label.toLowerCase()}`)
                                  }
                                ...
                                onClick={() => {
                                  ...
                                  if (canvas) {
                                    ...
                                    if (ctx) {
                                      ...
                                      handleSignatureChange(field.id, null)
                                    }
                                  }
                                }}
                              >
      ...
      <Footer className="text-center">
        <div className="text-gray-500">{formData.settings?.footerText || "Powered by ROOM Form Builder"}</div>
      </Footer>
    </Layout>
  )
}
