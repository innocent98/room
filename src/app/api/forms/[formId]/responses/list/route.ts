...
export async function GET(request: NextRequest, { params }: { params: { formId: string } }) {
  try {
    ...
    let filteredResponses = responses.map((response) => {
      ...
      const isComplete = requiredFieldIds.every((fieldId) => answeredFieldIds.includes(fieldId))
      ...
      filteredResponses = filteredResponses.filter((response) => {
        ...
        return Object.values(response.answers).some((answer) => {
          ...
          if (typeof value === "string") {
            ...
            return value.some((v) => typeof v === "string" && v.toLowerCase().includes(search.toLowerCase()))
          }
          return false
        })
      })
    ...
    if (answerType) {
      switch (answerType) {
        case "complete":
          filteredResponses = filteredResponses.filter((response) => response.isComplete)
          break
        case "incomplete":
          filteredResponses = filteredResponses.filter((response) => !response.isComplete)
          break
    ...
    const filteredCount = search || answerType ? filteredResponses.length : totalCount
    ...
    console.error(`Error fetching responses for form ${params.formId}:`, error)
