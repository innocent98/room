...
export async function GET(request: NextRequest, { params }: { params: { formId: string } }) {
  try {
    ...
    const requiredFieldIds = formWithFields.fields.filter((field) => field.required).map((field) => field.id)

    const completeResponses = responses.filter((response) => {
      const answeredFieldIds = response.answers.map((answer) => answer.fieldId)
      return requiredFieldIds.every((fieldId) => answeredFieldIds.includes(fieldId))
    })

    const completionRate = Math.round((completeResponses.length / totalResponses) * 100)
    ...
    const questionStats = calculateQuestionStats(formWithFields.fields, responses)
...
function calculateQuestionStats(fields: any[], responses: any[]) {
  const questions = fields.map((field) => {
    ...
    const answers = responses.flatMap((response) => response.answers.filter((answer:any) => answer.fieldId === field.id))
    ...
    if (field.type === "radio" || field.type === "checkbox" || field.type === "dropdown") {
      ...
      const optionCounts:any = new Map(options.map((option:any) => [option, 0]))
        ...
        try {
          ...
          if (answer.value.startsWith("[") && answer.value.endsWith("]")) {
            ...
            if (optionCounts.has(answer.value)) {
              optionCounts.set(answer.value, optionCounts.get(answer.value) + 1)
            }
          }
        } catch (e) {
      ...
      if (answerDistribution.length > 0) {
        const maxCount = Math.max(...answerDistribution.map((item:any) => item.count))
        const mostCommon = answerDistribution.find((item:any) => item.count === maxCount)
  ...
  return {
    totalAnswered: responses.reduce((sum, response) => sum + response.answers.length, 0),
    questions,
  }
}
