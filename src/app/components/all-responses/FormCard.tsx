...
import { EyeOutlined, BarChartOutlined, SettingOutlined } from "@ant-design/icons"
...
export default function FormCard({ form }: FormCardProps) {
  ...
  return (
    <Card hoverable className="w-full transition-shadow duration-300 hover:shadow-lg">
      <div className="flex justify-between items-start">
        <div>
          <h2 className="text-lg font-semibold mb-2">{form.title}</h2>
          <p className="text-sm text-gray-500 mb-2">Created on: {form.creationDate}</p>
          <Badge status={getStatusColor(form.status) as any} text={form.status} />
        </div>
